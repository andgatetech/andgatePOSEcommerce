...
export default function CareersPage() {
  return (
    <div className="bg-(--color-bg) min-h-screen">
      ...
      <div className="bg-gradient-to-r from-(--color-primary-900) to-(--color-primary-dark) py-16 md:py-24 text-white text-center rounded-b-3xl">
        <span className="text-(--color-cta) font-bold tracking-widest uppercase text-sm mb-4 block">Careers</span>
        <h1 className="text-3xl md:text-6xl font-bold mb-6 px-4">Build the future of commerce</h1>
        <p className="text-base md:text-lg text-(--color-primary-100) max-w-2xl mx-auto px-4">
      ...
      <Container className="py-14 md:py-20">
        <div className="grid md:grid-cols-3 gap-8 mb-16 md:mb-20 -mt-10 md:-mt-20">
          <div className="bg-white p-6 md:p-10 rounded-3xl shadow-xl shadow-black/5 border border-(--color-primary-100) border-t-8 border-t-(--color-cta) flex flex-col items-center md:items-start text-center md:text-left">
          ...
          <div className="bg-white p-6 md:p-10 rounded-3xl shadow-xl shadow-black/5 border border-(--color-primary-100) border-t-8 border-t-(--color-primary) flex flex-col items-center md:items-start text-center md:text-left">
            <div className="bg-(--color-primary-100) p-5 rounded-2xl text-(--color-primary) mb-6">
        ...
        <div className="space-y-6">
          {[
            ...
            <div key={job.id} className="bg-white p-6 md:p-8 rounded-3xl shadow-md border border-(--color-border) flex flex-col gap-5 md:flex-row md:items-center justify-between hover:shadow-lg hover:border-(--color-primary) transition-all cursor-pointer">
              ...
              <button className="bg-(--color-primary) hover:bg-(--color-primary-dark) w-full md:w-auto text-white font-bold py-3.5 px-8 rounded-xl transition-colors whitespace-nowrap">
                Apply Now
              </button>
            </div>
          ))}
