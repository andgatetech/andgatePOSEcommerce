...
const steps = [
    {
        step: "01",
        eyebrow: "Discover",
        title: "Browse trusted local sellers",
        description: "Find grocery, fashion, beauty, electronics, and daily essentials from verified sellers in one organized marketplace.",
    ...
    },
    {
        ...
        description: "Add products to cart, choose cash on delivery or digital payment, then confirm your address without a long checkout maze.",
    ...
    },
    {
        ...
        description: "Stores prepare the parcel, courier partners move it, and shoppers can follow the order journey from their account.",
            ...
            <Container>
                <div className="relative mb-10 flex flex-col gap-5 text-center md:mb-12 md:flex-row md:items-end md:justify-between md:text-left">
                    <div className="mx-auto max-w-[720px] md:mx-0">
                        ...
                        <h2 className="mt-4 text-[32px] font-extrabold leading-[1.06] text-(--color-primary-900) sm:text-[44px]">
                    ...
                    <div className="mx-auto grid w-full max-w-[360px] grid-cols-3 gap-2 rounded-[8px] border border-(--color-primary-100) bg-white/80 p-2 shadow-[0_16px_36px_rgba(2,58,92,0.08)] backdrop-blur md:mx-0">
                        {["Choose", "Pay", "Track"].map((item) => (
                            <div key={item} className="rounded-[8px] bg-(--color-primary-50) px-3 py-3 text-center">
                ...
                <div className="relative grid gap-5 lg:grid-cols-3">
                    ...
                    {steps.map((step, index) => (
                        <article
                            key={step.step}
                            className="group relative overflow-hidden rounded-[8px] border border-(--color-primary-100) bg-white p-3 shadow-[0_18px_50px_rgba(2,58,92,0.08)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_28px_70px_rgba(2,58,92,0.13)]"
                        >
                            ...
                            <div className="relative h-[260px] overflow-hidden rounded-[8px] bg-(--color-primary-50)">
                                <Image
                                    src={step.image}
                                    alt={step.title}
                                    fill
                                ...
                                <div className="absolute bottom-4 left-4 right-4 rounded-[8px] border border-white/20 bg-white/88 p-4 shadow-[0_16px_34px_rgba(2,58,92,0.16)] backdrop-blur-md">
                                    <div className="flex items-start gap-3">
                                        <span className={["flex h-11 w-11 shrink-0 items-center justify-center rounded-[8px] text-white shadow-lg", step.accent].join(" ")}>
                                            <step.icon className="h-5 w-5" />
                                        </span>
                                        <div className="min-w-0">
                                            <h3 className="text-[18px] font-extrabold leading-tight text-(--color-primary-900)">
                                                {step.title}
                                            </h3>
                                            <p className="mt-1 text-[12px] font-bold leading-5 text-(--color-cta-dark)">{step.badge}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            {index === 2 && (
                                <div className="mx-3 mb-4 flex flex-wrap items-center gap-2 rounded-[8px] bg-(--color-primary-50) px-3 py-3">
